...
def robot_description(context: LaunchContext, robot_count, use_sim_time):
    ...
    for i in range(int(context.perform_substitution(robot_count))):
        robot_description = Command(['xacro ', 
                                    os.path.join(get_package_share_directory('simulation_bringup'), 
                                                'urdf', 
                                                'sensebeetle.xacro')])
        ...
        # communication_client_launch = PythonLaunchDescriptionSource(
        #     os.path.join(get_package_share_directory('communication_client'), 'launch', 'communication_clientlaunch.py'),
        #     launch_arguments={
        #         'robot_id': str(i),
        #         'port': 12132 + i,
        #     }.items()
...
def communication_client_launch(context: LaunchContext, robot_count):
    ...
    for i in range(int(context.perform_substitution(robot_count))):
        communication_client_launch = IncludeLaunchDescription(
            PythonLaunchDescriptionSource(os.path.join(get_package_share_directory('communication_client'), 
                                                       'launch', 
...
def world_launch(context: LaunchContext, world_name):
    ...
    world_launch = IncludeLaunchDescription(
        ...
        launch_arguments={
            'world': os.path.join(get_package_share_directory('simulation_bringup'), 'world', name, name + '.world')
        }.items(),
    )
    return [world_launch]
...
def generate_launch_description():
    ...
    communication_server_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(os.path.join(get_package_share_directory('communication_server'), 'launch', 'communication_server.launch.py'))
    )
    ...
    ld.add_action(OpaqueFunction(function=robot_description, args=[robot_count, use_sim_time]))
    ld.add_action(TimerAction(period=10.0, actions=[communication_server_launch]))
